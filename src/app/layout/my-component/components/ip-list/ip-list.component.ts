...
export class IpListComponent implements OnInit,OnChanges {
  ...
  ngOnInit() {
    ...
    changeIsForbid($event,id){
        //alert(id)
        this.systemService.updateIsForbid($event,id).subscribe(
            data=>{
    ...
    addIpMap(event,isBlack){
        ...
        this.systemService.addIpMap(event.formModel.value,isBlack).subscribe(
            data=>{
                if(data.errorCode===0){
    ...
    editIpMap(event,id){
       //console.log(event.formModel.value)
        ...
        this.systemService.updateIpMap(event.formModel.value,id).subscribe(
            data=>{
                //console.log(data)
                if(data.errorCode===0){
    ...
    deleteIpMap(id){
        ...
        this.modalService.confirm(confirmCfg).then((result) => {
            this.systemService.deleteIpMap(id).subscribe(
                data=>{
                    if(data.errorCode===0){
