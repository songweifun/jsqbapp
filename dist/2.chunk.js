...
var IpListComponent = (function () {
    ...
    IpListComponent.prototype.addIpMap = function (event, isBlack) {
        ...
        this.systemService.addIpMap(event.formModel.value, isBlack).subscribe(function (data) {
            if (data.errorCode === 0) {
                var alertCfg = new __WEBPACK_IMPORTED_MODULE_3__shared_components_modal_modal_model__["a" /* AlertConfig */](__WEBPACK_IMPORTED_MODULE_3__shared_components_modal_modal_model__["b" /* AlertType */].INFO, 'IP映射', '添加成功');
                ...
                var alertCfg = new __WEBPACK_IMPORTED_MODULE_3__shared_components_modal_modal_model__["a" /* AlertConfig */](__WEBPACK_IMPORTED_MODULE_3__shared_components_modal_modal_model__["b" /* AlertType */].ERROR, 'IP映射', data.msg);
                _this.modalService.alert(alertCfg);
            }
        });
    };
    IpListComponent.prototype.editIpMap = function (event, id) {
        ...
        this.systemService.updateIpMap(event.formModel.value, id).subscribe(function (data) {
            //console.log(data)
            if (data.errorCode === 0) {
                var alertCfg = new __WEBPACK_IMPORTED_MODULE_3__shared_components_modal_modal_model__["a" /* AlertConfig */](__WEBPACK_IMPORTED_MODULE_3__shared_components_modal_modal_model__["b" /* AlertType */].INFO, 'IP映射', '修改成功');
                _this.modalService.alert(alertCfg);
                event.ref.close();
                _this.appService.pageRefreshEventEmitter.emit(_this.transformPageData); //创建一个事件流发送传递过来的分页信息
            }
    ...
    IpListComponent.prototype.deleteIpMap = function (id) {
        var _this = this;
        var confirmCfg = new __WEBPACK_IMPORTED_MODULE_3__shared_components_modal_modal_model__["c" /* ConfirmConfig */]('您确定要删除此映射吗？');
        this.modalService.confirm(confirmCfg).then(function (result) {
            _this.systemService.deleteIpMap(id).subscribe(function (data) {
                if (data.errorCode === 0) {
                    var alertCfg = new __WEBPACK_IMPORTED_MODULE_3__shared_components_modal_modal_model__["a" /* AlertConfig */](__WEBPACK_IMPORTED_MODULE_3__shared_components_modal_modal_model__["b" /* AlertType */].INFO, 'IP映射', '删除成功');
                    _this.modalService.alert(alertCfg);
                    _this.appService.pageRefreshEventEmitter.emit(_this.transformPageData); //创建一个事件流发送传递过来的分页信息
                }
                else {
                    var alertCfg = new __WEBPACK_IMPORTED_MODULE_3__shared_components_modal_modal_model__["a" /* AlertConfig */](__WEBPACK_IMPORTED_MODULE_3__shared_components_modal_modal_model__["b" /* AlertType */].ERROR, 'IP映射', '删除失败');
                    _this.modalService.alert(alertCfg);
                }
            });
        }, function (reason) {
